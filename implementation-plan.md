...
#### LSP vs MCP verdict: **LSP** (all three sub-features)

All three sub-features validate string literals or constants in well-known method call patterns against runtime data that's already in memory. They fit the LSP's role as "fast, automatic first-line defense against common LLM hallucinations":

- **Module ID validation** (`Synth.getEffect("MyReverbb")`): String literal argument checked against the live module tree. This is a dictionary lookup during the shadow parse — fast and common hallucination target (LLMs guess module names constantly). The module tree is accessible from `HiseSpecialData` during the shadow parse, no new REST endpoint needed.
- **Component property validation** (`slider.set("value", 5)`): String literal in `.set()`, component type known via Tier 2 `constObjects` resolution, valid property list is finite per component type. Fits naturally into the `ApiValidationAnalyzer` path.
- **Module attribute validation** (`proc.setAttribute(proc.Intensity, 0.5)`): Constant resolution on the resolved object. Slightly harder than the string-literal cases because `proc.Intensity` is a constant lookup rather than a string, but the parser already handles constant resolution in the existing Tier 2 path.

**Mechanism:** Read from live `HiseSpecialData` during the shadow parse. The shadow parser already has full access to the runtime state via `ScopedDiagnosticParse`, and the module tree / component property lists are in-memory data structures. No REST queries, no caching layer, no additional network round-trips. This is the natural extension of Tier 2 resolution.
...
#### LSP vs MCP verdict: **LSP**

CSS validation is code correctness checking triggered by file edits — not a runtime query. The pattern is identical to the existing v1 API validation: detect a known method call (`setStyleSheet()`), extract the string literal argument, validate it against known rules (CSS parser), and return diagnostics. It's automatic, fast (CSS strings are small), and catches a real LLM error pattern — LLMs frequently hallucinate CSS property names, especially in a restricted subset like HISE's `simple_css`. The file-reference case (resolving a CSS file path and parsing its contents) is analogous to how `include()` resolution already works in the shadow parser.
...
#### LSP vs MCP verdict: **MCP** (with one LSP-side exception)

This is fundamentally different from the other v2 features. It's not validating code — it's inspecting live state. The agent asks "what is this variable right now?" as a deliberate query to understand behavior, debug issues, or verify its work. This is:

- **Not triggered by file edits** — it's a point-in-time query
- **Not about catching mistakes** — it's about understanding runtime state
- **Potentially expensive** — serializing full object trees with members and properties
- **Agent-initiated** — the agent decides when it needs this information

The three alternative approaches listed above should be narrowed to **MCP only** (dedicated REST endpoint, agent calls via MCP tool):

1. **LSP hover isn't used by AI agents today** — neither OpenCode nor Claude Code send `textDocument/hover` requests to LSP servers. Implementing hover in the LSP would be dead code for the primary use case (AI agents). If human editors become a target later, hover can be added then.
2. **The MCP server already has the pattern** — `get_component_properties`, `get_component_value`, and `list_components` are all agent-initiated runtime queries. A new `inspect_variable` tool extends this naturally.
3. **Pull diagnostics model (including variable state summaries with every diagnostic response) adds noise** — most of the time the agent doesn't need variable state. Sending it on every save wastes bandwidth in the diagnostic response and adds processing time to the fast-path LSP cycle.

**One LSP-side exception:** The shadow parser's Tier 2 resolution already determines the concrete type of `constObjects` during validation. As a cheap byproduct, the diagnostic response could include type annotations as info-level diagnostics or metadata (e.g., `[info] slider: ScriptSlider`). This gives the agent passive type awareness without a separate MCP query and costs almost nothing in the diagnostic pipeline. Full value/member/property inspection remains MCP-only.
...
### v2 Feature Routing Summary

The guiding principle: **if it validates code correctness from file content + runtime identifiers, it belongs in the LSP (fast, automatic, every save). If it queries "what is happening right now," it belongs in the MCP (deliberate, agent-initiated).**

| v2 Feature | LSP or MCP | Reasoning |
|---|---|---|
| Module ID validation | **LSP** | String literal check against module tree during shadow parse, most common LLM hallucination |
| Component property validation | **LSP** | String literal in `.set()`, type known via Tier 2, finite property list per component type |
| Module attribute validation | **LSP** | Constant resolution on resolved object, fits existing `ApiValidationAnalyzer` path |
| CSS validation (inline strings) | **LSP** | String literal in `setStyleSheet()`, existing `simple_css` parser, automatic |
| CSS validation (file references) | **LSP** | File path resolution + parse, analogous to `include()` handling |
| Variable type annotations | **LSP** (lightweight) | Cheap byproduct of Tier 2 resolution, passive type awareness at near-zero cost |
| Runtime state inspection (values, members, properties) | **MCP** | Deliberate query, not validation, potentially expensive, agent-initiated |
| Additional enrichment flags | **LSP** (data source) | Feeds into existing diagnostic pipeline, improves validation quality |

*Analysis from OpenCode integration testing session (Feb 2026). The key insight driving this split: the LSP's value is in the automatic edit→lint cycle with zero agent action. Anything that requires the agent to decide "I need this information now" is better served by MCP, where the agent explicitly calls a tool.*
